...
output "multi_arch_compute_ip_combo" {
  value = [for index, s in openstack_networking_port_v2.worker_port : "host worker-p-${index} { hardware ethernet ${s.mac_address}; fixed-address ${s.fixed_ip[0].ip_address}; }"]
}
