...
resource "openstack_compute_instance_v2" "worker" {
  depends_on        = [openstack_networking_port_v2.worker_port, openstack_compute_instance_v2.helper]
  count             = var.worker_count
  ...
  user_data = templatefile(
    ...
    {
      ignition_ip : openstack_networking_port_v2.helper_port.all_fixed_ips[0],
